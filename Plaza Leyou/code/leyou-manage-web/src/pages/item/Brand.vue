<template>
  <v-card style="background-color: #FFFBF0">
    <v-card-title>
      <v-btn class="mx-2" fab dark small color="lime lighten-1" @click="addBrand">
      <v-icon large dark color="grey lighten-3">add</v-icon>
      </v-btn>
      <v-btn class="mx-2" fab dark small color="light-blue darken-4" @click="deleteSelectedBrand">
        <v-icon large dark color="grey lighten-3"> delete</v-icon>
      </v-btn>
     <!-- <v-btn color="primary" @click="addBrand">Añadir marca</v-btn>
      <v-btn color="primary" @click="deleteSelectedBrand">Eliminar marca</v-btn>-->
      <!--搜索框，与search属性关联-->
      <v-spacer/>
      <v-flex xs3>
        <v-text-field label="Búsqueda con palabras clave" v-model.lazy="search" append-icon="search" hide-details/>
      </v-flex>
    </v-card-title>
    <v-divider/>
    <v-data-table
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="text-xs-center" style="background-color: #FFFBF0">{{ props.item.letter }}</td>
        <td class="justify-center layout px-0" style="background-color: #FFFBF0">
          <v-btn icon @click="editBrand(props.item)" color="teal accent-4">
            <i class="el-icon-edit"/>
          </v-btn>
          <v-btn icon @click="deleteBrand(props.item)" color="light-blue darken-3">
    ...
    <v-dialog max-width="500" v-model="show" persistent scrollable>
      <v-card style="background-color: #FFFBF0">
        ...
        <v-toolbar dense dark color="primary">
          <v-toolbar-title>{{isEdit ? 'Modificar' : 'Añadir'}} Marca</v-toolbar-title>
          <v-spacer/>
          <!--关闭窗口的按钮-->
        ...
        <v-card-text class="px-5" style="height:400px">
          <!--brand-form是brand的局部组件，brand中的oldBrand和isEdit属性发生变化时，brand-form中的数据也发生变化-->
    ...
    data() {
      return {
        ...
        headers: [
          ...
          {text: 'Letra inicial', align: 'center', value: 'letter', sortable: true,},
          {text: 'Operación', align: 'center', value: 'id', sortable: false}
        ],
        show: false,// 控制对话框的显示
        oldBrand: {}, // 回显被编辑的品牌数据
        isEdit: false, // 是否是编辑
      ...
      getDataFromServer() { // 从服务的加载数的方法。
        ...
        this.$http.get("/item/brand/page", {
          params: {
            ...
            rows: this.pagination.rowsPerPage< 0 ? this.totalBrands: this.pagination.rowsPerPage,// 每页大小
