<template>
  <v-stepper v-model="step" style="background-color: #FFFBF0">
    <v-stepper-header>
      <v-stepper-step :complete="step > 1" step="1">Información general</v-stepper-step>
      <v-divider/>
      <v-stepper-step :complete="step > 2" step="2">Descripción de producto</v-stepper-step>
      <v-divider/>
      <v-stepper-step :complete="step > 3" step="3">Especificaciones</v-stepper-step>
      <v-divider/>
      <v-stepper-step step="4">Atributos de SKU</v-stepper-step>
    </v-stepper-header>
    <v-stepper-items>
      <!--1、基本信息-->
      <v-stepper-content step="1">
        <v-flex class="xs10 mx-auto">
          <v-form v-model="valid" ref="basic">
            <v-layout row>
              <v-flex xs5>
                ...
                <v-cascader
                  ...
                  label="Elija categoría de producto, por favor"/>
              </v-flex>
              <v-spacer/>
              <v-flex xs5>
                <!--Marca-->
                <v-select
                  ...
                  item-text="name"
                  item-value="id"
                  label="Marca perteneciente"
                  v-model="goods.brandId"
                  required
                  autocomplete
                  clearable
                  dense chips
                  :rules="[v => !!v || 'Obligatorio de rellenar la marca']"
                >
            ...
            <v-text-field label="título de artículo" v-model="goods.title" :counter="200" required :rules="[v => !!v || 'Obligatorio de rellenar el título del artículo']" hide-details/>
            <v-text-field label="gancho comercial " v-model="goods.subTitle" :counter="200" hide-details/>
            <v-text-field label="parking list" v-model="goods.spuDetail.packingList" :counter="1000" multi-line :rows="3" hide-details/>
            <v-text-field label="servicio posventa" v-model="goods.spuDetail.afterService" :counter="1000" multi-line :rows="3" hide-details/>
          </v-form>
        </v-flex>
      </v-stepper-content>
      <!--2、商品描述-->
      ...
      <v-stepper-content step="3">
        <v-flex class="xs10 mx-auto px-3">
          <!--遍历整个规格参数-->
          <v-card class="my-2" style="background-color: #FFFBF0">
      ...
      <v-stepper-content step="4">
        <v-flex class="mx-auto">
          ...
          <v-card flat v-for="spec in specialSpecs" :key="spec.name">
            ...
            <v-card-text class="px-5">
              <div v-for="i in spec.options.length+1" :key="i" class="layout row px-5">
                <v-text-field :placeholder="spec.name + ' nuevo' + ':'" class="flex xs10" auto-grow
          ...
          <v-card class="elevation-0">
            <!--标题-->
            <div class="subheading py-3">Listado de SKU:</div>
            <v-divider/>
        ...
        <v-flex xs3 offset-xs8>
          <v-btn color="info" small right @click="submit">Guardar infos de artículo</v-btn>
        </v-flex>
      </v-stepper-content>
    </v-stepper-items>
  </v-stepper>
  ...
  methods: {
    submit() {
      ...
      if(!this.$refs.basic.validate){
        this.$message.error("¡Rellene los campos del formulario, por favor！");
      }
      ...
      this.$http({
        ...
        .then(() => {
          ...
          this.$emit("close");
          // 提示成功
          this.$message.success("¡Artículo guardado!");
        })
        .catch(() => {
